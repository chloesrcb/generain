...
# get threshold corresponding to quantile q in comephore
threshold <- quantile(comephore$p124, probs = q, na.rm = T)
...
get_mode_excess_dir <- function(x, quantile) {
  x <- na.omit(x)  # Remove NA values
  x_excesses <- x[x > quantile]
...
get_mean_excess_dir <- function(x, quantile) {
  x <- na.omit(x)  # Remove NA values
...
compute_wind_episode <- function(episode, delta, quantile) {
  ...
  Q0 <- names(episode)[which(sapply(episode, 
  ...
  if (nrow(wind_subset) > 0) {
    ...
    cardDir_excess <- get_mode_excess_dir(wind_subset$cardDir, quantile)
    DD_excess <- get_mean_excess_dir(wind_subset$DD, quantile)
...
wind_per_episode <- lapply(list_episodes, compute_wind_episode, delta = 15,
                          quantile = threshold)
...
ggplot(wind_ep_df, aes(x = cardDir_excess)) +
  geom_bar(color = "#797474", fill = btfgreen, alpha=0.5, width = 1) +  # Bar plot for wind direction
  coord_polar(start = 15*pi/8) +  # Convert to polar coordinates, starting at North
  labs(x = "Mean wind Direction in excess", y = "Count", title = "") +
  theme_minimal() +  # Clean theme
  theme(axis.text.x = element_text(size = 12, face = "bold"))  # Improve readability
...
wind_ep_df$Match <- wind_ep_df$cardDir == wind_ep_df$cardDirt0
