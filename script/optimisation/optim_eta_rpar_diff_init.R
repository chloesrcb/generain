...
# Choose initial values for eta1
eta1_values <- c(0.1, 0.5, 1, 2, 5)

# Créer une liste pour stocker les résultats
result_list <- list()

# Itérer sur les différentes valeurs de eta1
for (eta1 in eta1_values) {
  # Initialiser les paramètres avec eta1 variable et eta2 fixé à 1
  init_param <- c(params[1:4], eta1, 1)
  
  # Lancer la simulation (M=1, donc pas besoin de mclapply ici)
  result <- process_simulation(m = 1,
                               list_simu = list_rpar,
                               u = u,
                               list_lags = list_lags,
  ...
  # Stocker le résultat avec un nom clair
  result_list[[paste0("eta1_", eta1)]] <- result
}
