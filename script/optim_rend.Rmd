...
q_values <- seq(0.85, 0.95, by = 0.005) # quantiles
...
q <- 0.9
q_values <- seq(0.85, 0.88, 0.9, 0.92, 0.95) # quantiles
...
# get the RMSE
df_rmse <- data.frame(rmse_beta1 = sqrt((result_table$beta1 - true_param[1])^2),
                rmse_beta2 = sqrt((result_table$beta2 - true_param[2])^2),
                rmse_alpha1 = sqrt((result_table$alpha1 - true_param[3])^2),
                rmse_alpha2 = sqrt((result_table$alpha2 - true_param[4])^2))

kable(df_rmse, "latex", booktabs = TRUE,
      caption = "RMSE for each parameter for all simulations")  %>%
  kable_styling(latex_options = "H", 
...
for (q in q_values) {
  ...
  df_rmse <- data.frame(rmse_beta1 = sqrt(mean((result_table$beta1 - true_param[1])^2, na.rm = TRUE)),
                        rmse_beta2 = sqrt(mean((result_table$beta2 - true_param[2])^2, na.rm = TRUE)),
                        rmse_alpha1 = sqrt(mean((result_table$alpha1 - true_param[3])^2, na.rm = TRUE)),
                        rmse_alpha2 = sqrt(mean((result_table$alpha2 - true_param[4])^2, na.rm = TRUE)))
  
  rmse_list[[as.character(q)]] <- df_rmse
}

# Convert the RMSE list to a data frame for better display
df_rmse_quantiles <- do.call(rbind, rmse_list)

# Add quantile column
df_rmse_quantiles <- cbind(quantile = rep(q_values, each = 1), df_rmse_quantiles)

# Display RMSE for each quantile
kable(df_rmse_quantiles, "latex", booktabs = TRUE,
      caption = "RMSE for each parameter across quantiles") %>%
  kable_styling(latex_options = "H", 
                bootstrap_options = c("striped", "hover", 
                "condensed", "responsive"))
...
result_1 <- optim(c(0.4, 0.2, 1.5, 1), neg_ll, simu = simu, 
                df_lags = df_lags,
...
result_2 <- optim(c(0.2, 0.1, 1.5, 1), neg_ll_composite, list_simu = list_simu12,
