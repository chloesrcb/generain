...
for (t in 1:nrow(rain_hsm)) {
  ...
  for (i in 1:nrow(location_gauges)) {
    ...
    # Distances between X1 and each Comephore pixels
    distances <- distHaversine(cbind(coord_x_Xcenter, coord_y_Xcenter),
                              cbind(loc_px$Longitude, loc_px$Latitude))

    # 3 km around X1 ie 9 pixels
    cube_bounds <- loc_px[distances <= 1500, ]

    # Temporal bounds (3 hours around t_date) ie 3 observations by pixel
    t_start <- t_date - 3600 * 1.5
    t_end <- t_date + 3600 * 1.5
    filtered_comephore$date <- as.POSIXct(filtered_comephore$date,
                                format = "%Y-%m-%d %H:%M:%S", tz = "GMT")
    cube_obs <- filtered_comephore %>%
        filter(date >= t_start & date <= t_end) %>%
        dplyr::select(date, one_of(cube_bounds$pixel_name))

    if (nrow(cube_obs) == 0) next # Skip if no observations in the cube

    X_obs <- as.vector(as.matrix(cube_obs[, -1]))  # Remove the date column
    ...
    colnames(site_data)[(ncol(site_data) - 26):ncol(site_data)] <- paste0("X",
                                                                        1:27)
