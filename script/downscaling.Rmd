...
df_downscaling <- df_downscaling %>%
  separate(
    time,
    into = c("date", "hour_minute_second"),
    sep = " "
  ) %>%
  separate(
    date,
...
df_downscaling$coord_x_s <- location_gauges$Longitude[match(df_downscaling$lon, location_gauges$codestation)]
```
...
n_observations <- nrow(df_downscaling)
n_sites <- 17
n_predictors <- ncol(df_downscaling) - 1

X_all <- df_downscaling[, -c("Y_obs")]
...
X.X.nn.s <- X_all # Nn predictors for sigma

Y_obs <- df_downscaling$Y_obs
