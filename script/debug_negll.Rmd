  ...
  for (i in seq_len(n.BR)) { ## n=1 
    ...
    for (n in 2:N) {
      ...
      while(V > Z[N * (i - 1) + n]) {
        ...
        Y <- exp(W - W[n] - Varm1[, , , n])
        print(all(V * Y[seq_len(n-1)] < Z[(N*(i-1)+1):(N*(i-1)+(n-1))]))
...
empirical_excesses <- function(data_rain, quantile, df_lags) {
  ...
  for (t in unique_tau) { # Loop over temporal lags
    ...
    # Vectorize the outer loop over pairs of sites
    ind_s2 <- as.numeric(as.character(df_h_t$s2))
    ind_s1 <- df_h_t$s1

    for (i in seq_len(nrow(df_h_t))) { # Loop over each pair of sites
      # Get the data for the pair of sites
      rain_cp <- data_rain[, c(ind_s1[i], ind_s2[i]), drop = FALSE]
      rain_cp <- as.data.frame(na.omit(rain_cp)) # Remove NAs
      Tmax <- nrow(rain_cp) # Number of total observations

      if (Tmax <= t) next # Skip if there are not enough observations for lag

      # Lag the data
      rain_nolag <- rain_cp[1:(Tmax - t), 1]
      rain_lag <- rain_cp[(1 + t):Tmax, 2]

      Tobs <- length(rain_nolag) # Number of valid observations

      # Transform the data to uniform distributions
      rain_unif <- cbind(rank(rain_nolag) / (Tobs + 1),
                         rank(rain_lag) / (Tobs + 1))
      ...
      # Store Tobs and excesses in prefiltered rows
      idx <- excesses$s1 == ind_s1[i] & excesses$s2 == ind_s2[i] & excesses$tau == t
      excesses$Tobs[idx] <- Tobs
      excesses$kij[idx] <- joint_excesses
    }
  }
...
# for the same spatial lag and temporal lag
k_h_tau <- excesses[excesses$hnorm == 1 & excesses$tau == 1, ]
length(k_h_tau$kij)
# density plot of the number of excesses
