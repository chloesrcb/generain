...
results <- foreach(i = 1:num_iter, .combine = rbind) %dopar% {
  library(generain)
  BR <- generain::sim_BR(true_param[1]*2, true_param[2]*2, true_param[3], 
...
q_values <- seq(0.9, 0.96, by = 0.005) # quantiles
...
# beta1_values <- seq(0.1, 2.0, length.out = 50)
# beta2_values <- seq(0.1, 2.0, length.out = 50)

# # Create a matrix to store the log-likelihood values
# log_likelihood_matrix <- matrix(NA, nrow = length(beta1_values), ncol = length(beta2_values))

# # Loop over beta1 and beta2 values and compute the log-likelihood
# for (i in seq_along(beta1_values)) {
#   for (j in seq_along(beta2_values)) {
#     beta1 <- beta1_values[i]
#     beta2 <- beta2_values[j]
    
#     # Compute the negative log-likelihood for the current values of beta1 and beta2
#     log_likelihood_matrix[i, j] <- neg_ll_par_adv(beta1, beta2, alpha1, alpha2, adv1, adv2,
#                                                   simu_df_adv, df_lags, sites_coords)
#   }
# }

# par(mfrow = c(1, 1))
# # Plot the log-likelihood as a contour plot
# contour(beta1_values, beta2_values, log_likelihood_matrix,
#         xlab = "beta1", ylab = "beta2", main = "Log-Likelihood Contour Plot")
```
