...
file_path <- paste0(foldername, "br_", ngrid^2, "s_", length(temp), "t_", 1, ".csv")
...
Validation du modèle de Buhl avec WLSE:
```{r}
# get the distances
...
```{r excessplot, eval=FALSE, echo=FALSE}
q <- 0.9
...
par(mfrow = c(2, 3))
q_values <- c(0.8, 0.85, 0.9, 0.92, 0.95, 0.955)
for (q in q_values) {
...
q_values <- seq(0.92, 0.965, by = 0.005)
...
for (q in q_values) {
  ...
  x <- 0:Tobs_tau_h
  # Density
  plot(density(k_tau_h), main = paste("Density vs Binomial distribution (q =",
...
q_values <- seq(0.9, 0.95, by = 0.005)
par(mfrow = c(ceiling(length(q_values)/3), 3))
for (q in q_values) {
  excesses <- empirical_excesses(simu_df, quantile = q, df_lags = df_lags)
  n_marg <- get_marginal_excess(simu_df, quantile = q)
...
chi_theorical <- theorical_chi(true_param, df_lags)
chi_emp <- chispatemp_dt(simu_df, df_lags, quantile = q)
tau_values <- unique(excesses$tau)
...
Ici on fixe le quantile à 0.92 et $h = 3$ et on fait varier le lag temporel $\tau$:
...
q_values <- c(0.9, 0.91, 0.92, 0.93, 0.94, 0.95) # quantiles
...
```{r fix3params1, echo=FALSE}
quantiles <- seq(0.75, 0.95, 0.01)
...
```{r,  echo=FALSE}
quantiles <- c(0.93, 0.94, 0.945, 0.95, 0.955, 0.96)
...
plot(density(kij), main = "Density of joint excesses")
```
