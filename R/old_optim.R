...
#'
#' @export
process_simulation <- function(i, M, m, list_simuM, u, list_lags, t0,
                        init_params, list_excesses, hmax = NA, wind_df = NA) {
  # Get the m corresponding simulations from list_simu inside a list
  list_episodes <- list_simuM[((i - 1) * m + 1):(i * m)]
  list_excesses <- list_excesses[((i - 1) * m + 1):(i * m)]
  list_lags <- list_lags[((i - 1) * m + 1):(i * m)]
